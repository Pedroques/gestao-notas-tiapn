    ...
    async function buscarParcelas() {
      ...
      try {
        // Busca parcelas e vendedores ao mesmo tempo
        const [resParcelas, resVendedores] = await Promise.all([
          fetch(`${BASE_URL}/parcelas`),
          fetch(`${BASE_URL}/vendedor`)
        ]);
        const parcelas = await resParcelas.json();
        const vendedores = await resVendedores.json();

        // Mapa id -> nome do vendedor para exibicao na tabela
        const mapaVendedor = {};
        vendedores.forEach(v => { mapaVendedor[v.id] = v.nome; });

        // Aplica filtros: periodo (opcional), vendedor e status
        const filtradas = parcelas.filter(p => {
          const venc = new Date(p.dataVencimento);
          const noPeriodo = (!dataIni || venc >= new Date(dataIni)) &&
            (!dataFim || venc <= new Date(dataFim));
          const doVendedor = idVendedor ? p.idVendedor === idVendedor : true;
          const doStatus = status ? p.status === status : true;
        ...
        filtradas.forEach(p => {
          ...
          // Calcula o status automaticamente pela data de vencimento
          // Ignora o status salvo no JSON para evitar dados desatualizados
          const hoje = new Date(); hoje.setHours(0, 0, 0, 0);
          const vencimento = new Date(p.dataVencimento); vencimento.setHours(0, 0, 0, 0);
          const diffDias = Math.ceil((vencimento - hoje) / (1000 * 60 * 60 * 24));

          // Atrasado: venceu antes de hoje
          // Vencendo: vence nos proximos 7 dias
          // Em dia: vence depois de 7 dias
          const statusReal = diffDias < 0
            ? 'Atrasado'
            : diffDias <= 7
              ? 'Vencendo'
              : 'Em dia';

          const badge = statusReal === 'Atrasado'
